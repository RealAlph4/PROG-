...
class App(ctk.CTk):
    ...
    def init_pedidos_tab(self):
        ...
        ctk.CTkButton(history_btn_frame, text="🗑️ Borrar Seleccionado", command=self.borrar_pedido_seleccionado, 
                      fg_color="#D32F2F", hover_color="#B71C1C").pack(side="left", padx=15)
    ...
    def borrar_pedido_seleccionado(self):
        """
        Elimina el pedido seleccionado del historial sin pedir confirmación.
        """
        ...
        # La confirmación ha sido eliminada. Se procede a borrar directamente.
        try:
            if eliminar_pedido_por_id(self.pedido_seleccionado_id):
                # Opcional: mostrar un mensaje de éxito, o simplemente actualizar la lista
                print(f"Pedido con ID {self.pedido_seleccionado_id} eliminado exitosamente.")
                self.ver_pedidos() # Actualiza la tabla para reflejar el cambio
            else:
