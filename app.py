...
import json
import os # Importar os para abrir el PDF
...
# Se importa la función específica para generar la boleta desde el CRUD
from crud.pedido_crud import crear_pedido, obtener_pedidos, generar_boleta_pdf_from_db_pedido
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Se inicializa el contenido de cada pestaña
        self.init_clientes_tab()
        self.init_ingredientes_tab()
        self.init_menus_tab()
        self.init_pedidos_tab()

    # --- PESTAÑA CLIENTES (Sin cambios funcionales) ---
    ...
    def agregar_cliente(self):
        ...
        self.cargar_clientes()
        self.cargar_comboboxes() # Actualiza el combo en pedidos
    ...
    # --- PESTAÑA INGREDIENTES (Sin cambios funcionales) ---
    ...
    # --- PESTAÑA MENÚS (Sin cambios funcionales) ---
    ...
    def init_pedidos_tab(self):
        ...
        ctk.CTkLabel(top_frame, text="Menú:").grid(row=0, column=2, padx=5, pady=5)
        self.combo_menus = ttk.Combobox(top_frame, state="readonly", width=30)
        self.combo_menus.grid(row=0, column=3, padx=5, pady=5)
        self.entry_cant = ctk.CTkEntry(top_frame, placeholder_text="Cant.", width=70)
        self.entry_cant.grid(row=0, column=4, padx=5, pady=5)
        ctk.CTkButton(top_frame, text="Añadir Item", command=self.anadir_item_pedido).grid(row=0, column=5, padx=5, pady=5)
        ...
        vsb2 = ttk.Scrollbar(tree_hist_frame, orient="vertical", command=self.tree_pedidos.yview)
        ...
        # Datos iniciales
        self.carrito = []  # Formato: [(menu_id, cantidad), ...]
        self.cargar_comboboxes()
        self.ver_pedidos()
    ...
    def anadir_item_pedido(self):
        ...
        # Chequear si el item ya está en el carrito para sumarlo
    ...
    def actualizar_tree_items(self):
        # Limpiar el treeview del carrito
        for i in self.tree_items.get_children(): self.tree_items.delete(i)
        
        # Volver a llenarlo con los datos actualizados del carrito
    ...
    def guardar_pedido(self):
        if not self.combo_clientes.get():
            messagebox.showwarning("Aviso", "Seleccione un cliente"); return
        if not self.carrito:
            messagebox.showwarning("Aviso", "Añada items al pedido"); return
        ...
        # Resetear UI para un nuevo pedido
        self.carrito.clear()
        self.actualizar_tree_items()
        self.actualizar_total()
        self.ver_pedidos()  # Actualiza la lista del historial
    ...
    def generar_boleta_seleccionada(self):
        if self.pedido_seleccionado_id is None:
            messagebox.showwarning("Sin selección", "Por favor, seleccione un pedido del historial para generar su boleta.")
            return
        ...
        try:
            ...
            messagebox.showerror("Error Inesperado", f"Ocurrió un error al generar el PDF: {e}")
    ...
    def ver_pedidos(self):
        ...
        for p in sorted(peds, key=lambda x: x.fecha, reverse=True):
            ...
            self.tree_pedidos.insert('', 'end', iid=p.id, values=(
                p.id, 
                nombre_cliente, 
                f"${p.total:,.0f}", 
...
if __name__ == "__main__":
    # Asegurarse de que el directorio de boletas existe
